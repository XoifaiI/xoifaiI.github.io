...
flowchart LR
    subgraph Alice
        A1[Generate Secret] --> A2[Mask Key] --> A3[Get Public]
    end
    
    subgraph Bob
        B1[Generate Secret] --> B2[Mask Key] --> B3[Get Public]
    end
    
    A3 -->|AlicePublic| B4[Exchange]
    B3 -->|BobPublic| A4[Exchange]
    
    A4 --> S[Same Shared Secret]
    B4 --> S
...
flowchart LR
    subgraph Setup
        Gen[Generate Secret] --> Mask --> Public[Get Public Key]
    end
    
    Public -->|EphemeralPublic| Exchange
    Peer[PeerEphemeralPublic] --> Exchange
    
    subgraph Derive["Key Derivation"]
        Exchange --> Shared[SessionShared1]
        Shared --> Blake3[Blake3.DeriveKey]
        Context[Context: session key] --> Blake3
        Blake3 --> Key[SessionKey 32 bytes]
    end
